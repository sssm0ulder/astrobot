...
async def moon_sign_menu_button_handler(
    ...
    await moon_in_sign_menu(message, state, keyboards, database, event_from_user)
...
async def moon_in_sign_menu_callback_handler(
    ...
    await moon_in_sign_menu(
        callback.message, state, keyboards, database, event_from_user
    )


@r.callback_query(
    MainMenu.moon_in_sign, F.data.in_([bt.favorable, bt.unfavorable, bt.general])
)
...
async def moon_in_sign_menu(
    ...
    photo = BufferedInputFile(file=photo_bytes, filename=FileName.MOON_SIGN.value)

    await message.answer_photo(photo=photo)
    bot_message = await message.answer(text, reply_markup=keyboards.moon_in_sign_menu)
