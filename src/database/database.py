...
from src.database.models import (Base, CardOfDay, GeneralPrediction,
                                 Interpretation, Location, Payment, Promocode,
                                 User, ViewedPrediction)
...
class Database:
    ...
    def update_user_current_location(self, user_id: int, new_location: Location):
        ...
        if user:
            ...
            old_location = (
                self.session.query(Location).filter_by(id=old_location_id).first()
            )
            if old_location:
    ...
    def update_location(self, location: Location):
        existing_location = (
            self.session.query(Location).filter_by(id=location.id).first()
        )
    ...
    def get_interpretation(
        ...
        interpretation = (
            self.session.query(Interpretation)
            .filter_by(
    ...
    def get_viewed_predictions_by_user(self, user_id: int) -> list:
        predictions = (
            self.session.query(ViewedPrediction).filter_by(user_id=user_id).all()
        )
    ...
    def get_unviewed_predictions_count(self, user_id: int) -> int:
        ...
        current_location = (
            self.session.query(Location).filter_by(id=user.current_location_id).first()
        )
        ...
        viewed_dates_strings = (
            self.session.query(ViewedPrediction.prediction_date)
    ...
    def delete_viewed_prediction(self, user_id: int, prediction_date: str):
        prediction = (
            self.session.query(ViewedPrediction)
    ...
    def delete_card_of_day(self, message_id: int) -> None:
        card_of_day = (
            self.session.query(CardOfDay).filter_by(message_id=message_id).first()
        )
        if card_of_day:
    ...
    def update_promocode(self, promocode_str: str, **kwargs):
        try:
            ...
            promocode = (
                self.session.query(Promocode).filter_by(promocode=promocode_str).first()
            )
